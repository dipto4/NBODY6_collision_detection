...
*
     ...
     &                ECOLL1,RCOLL,QPERI,ISTAR(NMX),ICOLL,ISYNC,NDISS1
      ...
      CHARACTER*8  WHICH1
      CHARACTER*64 ICOLLOUTFILE
      LOGICAL FIRST
      SAVE FIRST
      DATA FIRST /.TRUE./
      
      PRINT*, "DIPTCALLED: NSYS: ",NSYS
      PRINT*, "DIPTCALLED: TIME: ",TTOT 
*
*
      call xbpredall
...
*       Obtain mass loss and evolution epoch of composite star.
      DM = 0.0D0
      PRINT *, "DIPTTIME:",TTOT
      PRINT *, "DIPTTOTALMASS:",BODY(I1)*ZMBAR,BODY(I2)*ZMBAR
      PRINT *, "DIPTRADII:",RADIUS(I1)*SU,RADIUS(I2)*SU
*       Print all the particles for 5 timesteps before the collision
*       took place
      IFCOL = IQREAR
      !IREARCOLL = IQREARCOLL
      
      !PRINT*, "IQFRONT=",IQFRONT
      !PRINT*, "IQREAR=",IQREAR

      !PRINT*, "IQFRONT(0)",IENCOUNTERQ(IQFRONT)
      !PRINT*, "IQFRONT(1)",IENCOUNTERQ(IQFRONT+1)
      !PRINT*, "IQFRONT(2)",IENCOUNTERQ(IQFRONT+2)
      !PRINT*, "IQFRONT(3)",IENCOUNTERQ(IQFRONT+3)
      !PRINT*, "IQFRONT(4)",IENCOUNTERQ(IQFRONT+4)
      !PRINT*, "IQFRONT(5)",IENCOUNTERQ(IQFRONT+5)
      DO I = 0, 5
        ICL = MODULO((IFCOL + I), 6)
        WRITE(ICOLLOUTFILE,169) ICOLLHAPPENED, I
 169    FORMAT('collision_track.469_',i2.2,'_',i1.1)

        OPEN(UNIT=469, STATUS='UNKNOWN',
     &   FORM='FORMATTED',FILE=ICOLLOUTFILE)

        DO J = 1, (N+NCH)
            WRITE(469,*) IQNAME(ICL,J),X1Q(ICL,J)*RBAR,X2Q(ICL,J)*RBAR,
     &      X3Q(ICL,J)*RBAR, V1Q(ICL,J)*VSTAR, V2Q(ICL,J)*VSTAR,
     &      V3Q(ICL,J)*VSTAR, BODY(J)*ZMBAR, QTIME(ICL)
        END DO
      END DO
      
      !ICOLLHAPPENED = ICOLLHAPPENED + 1
...
*       Ensure NAME of the heaviest body is new progenitor.
      IF (BODY(I2).GT.BODY(I1)) THEN
         ...
         NAME(I2) = NAM1
      END IF
*       Printing the details of the collision/before the collison here
*       coldata.69
        WRITE(69,*) ICOLLHAPPENED,TTOT, NAME(I1), NAME(I2), 
     &   BODY(I1)*ZMBAR, 
     &   BODY(I2)*ZMBAR,
...
*     Add ghost particle into NXTLST
     ...
     &                                  + (X(3,I2) - RDENS(3))**2)
      ...
      PRINT *,"DIPTR1RADIUS:",R1
      PRINT *,"DIPTR2RADIUS:",R2
*
      DO 20 K = 1,3
          X(K,I1) = CM(K)
          X0(K,I1) = CM(K)
